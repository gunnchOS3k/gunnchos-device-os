"""CX2F fail-closed authority and truth tests."""
...
from gunnchos_device_os.cx2f.lock import FOREIGN_BUSY, acquire_lock, read_lock, release_lock
from gunnchos_device_os.cx2f.paths import DEVICE_LAB_FORBIDDEN, evidence_root
from gunnchos_device_os.cx2f.session import ppm_diff
...
def test_cloud_kernel_cannot_earn_non_cloud_pass():
    ...
    t.CX2F_NON_CLOUD_KERNEL_BOOT_PASS = False
    assert t.CX2F_NON_CLOUD_KERNEL_BOOT_PASS is False
    assert t.gate_shell_stack() is False


def test_missing_dri_cannot_earn_drm_pass():
    t = Cx2fTokens(CX2F_DRM_CARD_PASS=False)
    assert t.CX2F_DRM_CARD_PASS is False
    assert t.gate_shell_stack() is False


def test_headless_weston_cannot_earn_drm_pass():
    t = Cx2fTokens(
        CX2F_WESTON_DRM_PASS=False,
        CX2F_WESTON_HEADLESS_FALLBACK_USED=True,
    )
    assert t.CX2F_WESTON_DRM_PASS is False
    assert t.CX2F_WESTON_HEADLESS_FALLBACK_USED is True
    assert t.gate_shell_stack() is False


def test_pid_without_framebuffer_change_no_render_pass():
    t = Cx2fTokens(CX2F_GUNNCH_SHELL_RENDER_PASS=False)
    # process alive alone is insufficient
    assert t.CX2F_GUNNCH_SHELL_RENDER_PASS is False


def test_assets_without_loaded_page_no_render_pass():
    t = Cx2fTokens(CX2F_SHELL_ASSET_DELIVERY_PASS=True, CX2F_GUNNCH_SHELL_RENDER_PASS=False)
    assert t.gate_shell_stack() is False


def test_browser_only_screenshot_insufficient():
    t = Cx2fTokens(CX2F_QEMU_FRAMEBUFFER_CAPTURE_PASS=False, CX2F_REAL_SCREEN_CAPTURE_PASS=False)
    assert t.CX2F_REAL_SCREEN_CAPTURE_PASS is False


def test_input_without_mutation_no_pass():
    t = Cx2fTokens(CX2F_REAL_INPUT_TO_SHELL_MUTATION_PASS=False)
    assert t.CX2F_REAL_INPUT_TO_SHELL_MUTATION_PASS is False


def test_stale_cx2e_evidence_not_cx2f_pass():
    cx2e = REPO / "artifacts/complete_experience/cx2e/CX2E_TOKENS.json"
    assert cx2e.is_file()
    # CX2F evidence root is distinct
    assert evidence_root(REPO).name == "cx2f"
    t = Cx2fTokens()
    assert t.CX2F_GUNNCH_SHELL_RENDER_PASS is False
...
def test_foreign_lock_token():
    assert FOREIGN_BUSY == "CX2F_QEMU_SLOT_BUSY_FOREIGN_PROCESS"
