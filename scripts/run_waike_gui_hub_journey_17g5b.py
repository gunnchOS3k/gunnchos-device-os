#!/usr/bin/env python3
"""17G.5B: WAIKE accepted-main #12 re-freeze + real GUI/Hub journey re-earn."""
...
def main() -> int:
    ...
    hist = WAIKE / "history" / f"pre_17g5b_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}"
    ...
    (WAIKE / "STORAGE_17G5B.json").write_text(json.dumps(storage, indent=2) + "\n")
    ...
    log_path = WAIKE / "WAIKE_RUN_LOG_17G5B.txt"
    ...
    gate = {
        ...
        "waike_pr12_merge_sha": ACCEPTED_WAIKE_LP_SHA,
        "hub_endpoint_policy_v1": True,
    }
    (OUT / "WAIKE_REAL_RUNTIME_DEVICE_LAB_PASS.json").write_text(
        json.dumps(gate, indent=2) + "\n", encoding="utf-8"
    )
    ...
    regression = {
        "schema": "gunnchos.device_lab.shared_runtime_regression_17g5b.v1",
        "generated_at_utc": _utc(),
        ...
        "note": (
            "17G.5B WAIKE #12 hub-policy re-freeze; LIVE/DSXL/RING/FOUR_GAME retained via "
            "compact rebind (product sources unchanged). ECO010 remains deferred."
        ),
    }
    (WAIKE / "SHARED_RUNTIME_REGRESSION_17G5B.json").write_text(
    ...
    master.update(
        {
            ...
            "prompt_17g5b_waike_pr12_refreeze_gui_hub_reearn": True,
