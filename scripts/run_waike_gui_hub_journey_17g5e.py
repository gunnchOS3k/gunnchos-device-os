#!/usr/bin/env python3
"""17G.5E: CSP-aware WAIKE accepted-main re-freeze + full GUI/Hub journey re-earn."""
...
PROMPT = "17G.5E"
EXPECTED_DEVICE_OS_HEAD = "18ea9a0ba691d1191d051d3a1ec6fe733bc14d0c"
...
def focused_shared_runtime_regression(*, waike_pass: bool, preflight: bool) -> dict:
    ...
    return {
        "schema": "gunnchos.device_lab.shared_runtime_regression_17g5e.v1",
        ...
        "note": (
            "17G.5E focused shared-runtime regression after CSP accepted-main re-freeze; "
...
def main() -> int:
    try:
        ...
        crash = {
            ...
            "NEXT_GATE": "DEVICE_OS_134_WAIKE_CSP_GUI_HUB_REEARN",
        ...
        try:
            ...
            (WAIKE / "WAIKE_RUN_17G5E_CRASH.txt").write_text(
...
def _main_impl() -> int:
    ...
    _write(WAIKE / "17G5E_PREFLIGHT.json", preflight)
    ...
    _write(WAIKE / "17G5E_QEMU_SLOT.json", slot)
    if not slot.get("ok"):
        gate = {
            "generated_at_utc": _utc(),
            "prompt": PROMPT,
            "WAIKE_REAL_RUNTIME_DEVICE_LAB_PASS": False,
            "verdict": "FAIL",
            "blocker": slot.get("blocker") or "17G5E_QEMU_SLOT_BUSY",
            "NEXT_GATE": "17G5E_QEMU_SLOT_BUSY",
        }
        _write(OUT / "WAIKE_REAL_RUNTIME_DEVICE_LAB_PASS.json", gate)
        print(json.dumps(gate, indent=2))
        return 3

    hist = WAIKE / "history" / f"pre_17g5e_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}"
    ...
    _write(WAIKE / "STORAGE_17G5E.json", storage)
    ...
    log_path = WAIKE / "WAIKE_RUN_LOG_17G5E.txt"
    ...
    gate = {
        ...
        "rejected_prior_artifact_sha256": "3df1e42e5d6c1b8b7e8ee31a636b2a6bec49120f38b6d28bd290b8a6e3fa1207",
        "drift_reason": "WAIKE_RUNTIME_CSP_EXACT_HUB_ORIGIN_ACCEPTED_MAIN",
        "artifact_source_sha": ACCEPTED_WAIKE_LP_SHA,
        "artifact_sha256": MAIN_AARCH64_GLIBC236_SHA256,
        "mandatory_17g5e": result.get("mandatory_17g5e"),
        "17g5e_evidence": result.get("17g5e_evidence"),
    }
    _write(OUT / "WAIKE_REAL_RUNTIME_DEVICE_LAB_PASS.json", gate)
    ...
    _write(WAIKE / "SHARED_RUNTIME_REGRESSION_17G5E.json", regression)

    firewall = expansion_firewall_proof()
    _write(WAIKE / "EXPANSION_FIREWALL_17G5E.json", firewall)
    ...
    master.update(
        {
            ...
            "prompt_17g5e_waike_csp_accepted_main_refreeze_gui_hub_reearn": True,
