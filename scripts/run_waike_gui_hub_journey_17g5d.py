#!/usr/bin/env python3
"""17G.5D: Full WAIKE real GUI/Hub journey re-earn on scoped GuestServiceForward."""
...
PROMPT = "17G.5D"
EXPECTED_DEVICE_OS_HEAD = "5ce7cb3e857b5b90cc19c939fb2d21831641a47b"
EXPECTED_PORTAL_HEAD = "b78083d233a169ff7245e828b0e368ece1fa2260"
PORTAL_WT = Path(
...
def focused_shared_runtime_regression(*, waike_pass: bool, preflight: bool) -> dict:
    ...
    return {
        "schema": "gunnchos.device_lab.shared_runtime_regression_17g5d.v1",
        ...
        "note": (
            "17G.5D focused shared-runtime regression after GuestServiceForward; "
...
def main() -> int:
    try:
        ...
        crash = {
            ...
            "verdict": "FAIL",
            "blocker": f"17G5D_RUNNER_EXCEPTION:{type(exc).__name__}:{exc}",
            "NEXT_GATE": "DEVICE_OS_134_WAIKE_GUI_LEARNER_DEPTH",
        ...
        try:
            ...
            (WAIKE / "WAIKE_RUN_17G5D_CRASH.txt").write_text(
...
def _main_impl() -> int:
    ...
    _write(WAIKE / "17G5D_PREFLIGHT.json", preflight)
    ...
    _write(WAIKE / "17G5D_QEMU_SLOT.json", slot)
    if not slot.get("ok"):
        gate = {
            "generated_at_utc": _utc(),
            "prompt": PROMPT,
            "WAIKE_REAL_RUNTIME_DEVICE_LAB_PASS": False,
            "verdict": "FAIL",
            "blocker": slot.get("blocker") or "17G5D_QEMU_SLOT_BUSY",
            "NEXT_GATE": "17G5D_QEMU_SLOT_BUSY",
        }
        _write(OUT / "WAIKE_REAL_RUNTIME_DEVICE_LAB_PASS.json", gate)
        print(json.dumps(gate, indent=2))
        return 3

    hist = WAIKE / "history" / f"pre_17g5d_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}"
    ...
    _write(WAIKE / "STORAGE_17G5D.json", storage)
    ...
    log_path = WAIKE / "WAIKE_RUN_LOG_17G5D.txt"
    ...
    gate = {
        ...
        "artifact_sha256": MAIN_AARCH64_GLIBC236_SHA256,
        "mandatory_17g5d": result.get("mandatory_17g5d"),
        "17g5d_evidence": result.get("17g5d_evidence"),
    }
    _write(OUT / "WAIKE_REAL_RUNTIME_DEVICE_LAB_PASS.json", gate)
    ...
    _write(WAIKE / "SHARED_RUNTIME_REGRESSION_17G5D.json", regression)

    firewall = expansion_firewall_proof()
    _write(WAIKE / "EXPANSION_FIREWALL_17G5D.json", firewall)
    ...
    master.update(
        {
            ...
            "prompt_17g5d_full_waike_gui_hub_reearn": True,
