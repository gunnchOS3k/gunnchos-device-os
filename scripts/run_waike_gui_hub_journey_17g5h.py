#!/usr/bin/env python3
"""17G.5H: WAIKE #16 accepted-main re-freeze + custom-protocol embed + full GUI/Hub re-earn."""
...
def focused_shared_runtime_regression(*, waike_pass: bool, preflight: bool) -> dict:
    ...
    return {
        "schema": "gunnchos.device_lab.shared_runtime_regression_17g5h.v1",
        ...
        "note": (
            "17G.5H focused shared-runtime regression after custom-protocol embed accepted-main re-freeze; "
...
def main() -> int:
    try:
        ...
        crash = {
            ...
            "NEXT_GATE": "DEVICE_OS_134_WAIKE_PR16_EMBED_GUI_HUB_REEARN",
        ...
        try:
            ...
            (WAIKE / "WAIKE_RUN_17G5H_CRASH.txt").write_text(
...
def _main_impl() -> int:
    ...
    _write(WAIKE / "17G5H_PREFLIGHT.json", preflight)
    ...
    _write(WAIKE / "17G5H_QEMU_SLOT.json", slot)
    if not slot.get("ok"):
        gate = {
            "generated_at_utc": _utc(),
            "prompt": PROMPT,
    ...
    hist = WAIKE / "history" / f"pre_17g5h_journey_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}"
    ...
    _write(WAIKE / "STORAGE_17G5H.json", storage)
    ...
    log_path = WAIKE / "WAIKE_RUN_LOG_17G5H.txt"
    ...
    _write(WAIKE / "SHARED_RUNTIME_REGRESSION_17G5H.json", regression)

    firewall = expansion_firewall_proof()
    _write(WAIKE / "EXPANSION_FIREWALL_17G5H.json", firewall)
    ...
    master.update(
        {
            ...
            "prompt_17g5h_waike_custom_protocol_embed_accepted_main_refreeze_gui_hub_reearn": True,
