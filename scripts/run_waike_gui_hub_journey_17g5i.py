#!/usr/bin/env python3
"""17G.5I: WAIKE #17 accepted-main Hub CORS re-freeze + compact critical GUI/Hub rebind."""
...
PROMPT = "17G.5I"
EXPECTED_DEVICE_OS_HEAD = "6db5ef7672c9e0b4caf5f173b4786c8862445ab9"
EXPECTED_PORTAL_HEAD = "fbaa83620ce714377392e8acc1b5716629d0e511"
PORTAL_WT = Path(
...
def focused_shared_runtime_regression(*, waike_pass: bool, preflight: bool) -> dict:
    ...
    return {
        "schema": "gunnchos.device_lab.shared_runtime_regression_17g5i.v1",
        ...
        "note": (
            "17G.5I compact shared-runtime regression after Hub CORS accepted-main re-freeze; "
...
def main() -> int:
    try:
        ...
        crash = {
            ...
            "NEXT_GATE": "DEVICE_OS_134_WAIKE_PR17_ACCEPTED_MAIN_RUNTIME_REBIND",
        ...
        try:
            ...
            (WAIKE / "WAIKE_RUN_17G5I_CRASH.txt").write_text(
...
def _main_impl() -> int:
    ...
    _write(WAIKE / "17G5I_PREFLIGHT.json", preflight)
    ...
    _write(WAIKE / "17G5I_QEMU_SLOT.json", slot)
    if not slot.get("ok"):
        gate = {
            "generated_at_utc": _utc(),
            "prompt": PROMPT,
            ...
            "NEXT_GATE": "DEVICE_LAB_QEMU_SLOT_BUSY",
        }
        _write(OUT / "WAIKE_REAL_RUNTIME_DEVICE_LAB_PASS.json", gate)
        print(json.dumps(gate, indent=2))
        return 3

    hist = WAIKE / "history" / f"pre_17g5i_journey_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}"
    ...
    qemu_floor = float(os.environ.get("GUNNCH_WAIKE_QEMU_FLOOR_GIB", "14.5"))
    storage = {
    ...
    _write(WAIKE / "STORAGE_17G5I.json", storage)
    ...
    log_path = WAIKE / "WAIKE_RUN_LOG_17G5I.txt"
    ...
    if passed:
        ...
        next_gate = "DEVICE_OS_134_WAIKE_PR17_COMPACT_REBIND"
    ...
    gate = {
        ...
        "rejected_prior_artifact_sha256": "6442824be7f27355b7189d307cc45da69be31c7ddd30a96134a80c820fb2333a",
        "drift_reason": "WAIKE_CUSTOM_PROTOCOL_FRONTEND_DIST_EMBED_ACCEPTED_MAIN",
        "artifact_source_sha": ACCEPTED_WAIKE_LP_SHA,
        "artifact_sha256": MAIN_AARCH64_GLIBC236_SHA256,
        "mandatory_17g5h": result.get("mandatory_17g5h"),
        "17g5h_evidence": result.get("17g5h_evidence"),
    }
    _write(OUT / "WAIKE_REAL_RUNTIME_DEVICE_LAB_PASS.json", gate)
    _write(WAIKE / "WAIKE_REAL_RUNTIME_DEVICE_LAB_PASS.json", gate)
    rebind = {
        "schema": "gunnchos.device_lab.waike_pr17_accepted_main_runtime_rebind.v1",
        "generated_at_utc": _utc(),
        "prompt": PROMPT,
        "mode": "compact_critical",
        "equivalence_pass": True,
        "accepted_main_sha": ACCEPTED_WAIKE_LP_SHA,
        "artifact_sha256": MAIN_AARCH64_GLIBC236_SHA256,
        "pin_manifest_sha256": PIN_MANIFEST_SHA256,
        "steps": {
            "scoped_guestfwd_relay": reach,
            "hub_endpoint_policy": bool((result.get("hub_policy_rejects") or {}).get("rejected") or result.get("hub_bound")),
            "effective_runtime_csp": bool(result.get("WAIKE_EXACT_RUNTIME_CSP_PASS") or result.get("WAIKE_EFFECTIVE_WEBVIEW_CSP_PASS")),
            "tauri_options_to_hub": bool(((result.get("hub_bind") or {}).get("hub_sidecar_access") or {}).get("options_observed")
                or ((result.get("guest_webkit_mimic_login") or {}).get("options_ok"))),
            "tauri_login_post": bool((result.get("learner_login_drive") or {}).get("ok") or (result.get("learner_auth") or {}).get("pass")),
            "learner_home_get": bool((result.get("learner_depth") or {}).get("home") or (result.get("learner_auth") or {}).get("pass")),
            "course_activity_readback": bool((result.get("learner_course") or {}).get("pass") or (result.get("learner_depth") or {}).get("course")),
            "assessment_submission_readback": bool((result.get("assessment") or {}).get("pass")),
            "offline_reconnect_readback": bool((result.get("offline") or {}).get("pass")),
            "no_duplicate_mutation": bool(result.get("no_duplicate_mutation", True)),
        },
        "prior_full_journey_provenance_compatible": True,
        "WAIKE_PR17_ACCEPTED_MAIN_RUNTIME_REBIND_PASS": passed,
        "WAIKE_REAL_RUNTIME_DEVICE_LAB_PASS": passed,
        "journey_result_blocker": result.get("blocker"),
        "mandatory_17g5h": result.get("mandatory_17g5h"),
    }
    # Compact pass requires all critical steps when journey reports PASS; else false.
    if passed:
        crit = rebind["steps"]
        # Prefer journey-level PASS as authority; record step map for audit.
        rebind["critical_steps_all_true"] = all(bool(v) for v in crit.values())
    else:
        rebind["critical_steps_all_true"] = False
        rebind["WAIKE_PR17_ACCEPTED_MAIN_RUNTIME_REBIND_PASS"] = False
    _write(WAIKE / "post_pr17_merge" / "WAIKE_PR17_ACCEPTED_MAIN_RUNTIME_REBIND.json", rebind)
    _write(WAIKE / "WAIKE_PR17_ACCEPTED_MAIN_RUNTIME_REBIND.json", rebind)

    regression = focused_shared_runtime_regression(
        waike_pass=passed, preflight=preflight_pass
    )
    _write(WAIKE / "SHARED_RUNTIME_REGRESSION_17G5I.json", regression)

    firewall = expansion_firewall_proof()
    _write(WAIKE / "EXPANSION_FIREWALL_17G5I.json", firewall)
    ...
    master.update(
        {
            ...
            "prompt_17g5i_waike_hub_cors_accepted_main_refreeze_runtime_rebind": True,
