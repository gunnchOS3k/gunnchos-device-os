"""CX2H.3 QEMU helpers — child overlay of CX2H.2; never kill foreign guests."""
...
from gunnchos_device_os.cx2h3.paths import (
    CX2H2_OVERLAY_REL,
    ensure_lab_tree,
    cx2h3_lab_root,
    repo_root_from_here,
...
def prepare_overlay(repo: Optional[Path] = None) -> Dict[str, Any]:
    ...
    overlay = lab / "overlays" / "cx2h3-aarch64.qcow2"
    parent = repo / CX2H2_OVERLAY_REL
    result: Dict[str, Any] = {"ok": False, "overlay": str(overlay), "parent": str(parent)}
    qimg = host_prereqs().get("qemu_img")
    if not qimg:
        result["blocker"] = "CX2H3_QEMU_IMG_MISSING"
        return result
    if not parent.is_file():
        result["blocker"] = "CX2H3_CX2H2_OVERLAY_MISSING"
        return result
    if overlay.is_file():
        result["ok"] = True
        result["reused"] = True
    ...
    if not result["ok"]:
        result["blocker"] = "CX2H3_OVERLAY_CREATE_FAILED"
    return result
...
def start_graphical_guest(repo: Optional[Path] = None, *, ssh_port: int = DEFAULT_SSH_PORT) -> Dict[str, Any]:
    ...
    work = lab / "work"
    result: Dict[str, Any] = {
        "schema": "gunnchos.cx2h3.graphical_boot.v1",
        "ok": False,
        "blocker": None,
        "ssh_port": ssh_port,
    }
    if cx_owned_qemu_running():
        ...
        while cx_owned_qemu_running() and time.time() < deadline:
            time.sleep(5)
        for pf in (Path("/tmp/cx2h3-graphical/qemu.pid"),):
            if pf.is_file():
                try:
                    ...
                    try:
                        ...
                        pass
        if cx_owned_qemu_running():
            result["blocker"] = "CX2H3_QEMU_SLOT_BUSY"
            return result
    ...
    lock = acquire_lock(
        branch="eng/cx2h3-browser-mail-offline-j2-j5",
        repo=str(repo),
        purpose="CX2H3_BROWSER_MAIL_OFFLINE",
    ...
    if proc.returncode != 0:
        ...
        result["blocker"] = f"CX2H3_GRAPHICAL_BOOT_FAILED: {proc.stderr[-2000:]}"
        return result
    time.sleep(1)
    ...
    lock_meta["wave"] = "CX2H.3"
    Path("/tmp/gunnchos-cx-qemu.lock").write_text(json.dumps(lock_meta, indent=2) + "\n")
    # Copy SSH keys from prior labs
    tmp_ssh = rt["base"] / "ssh"
    tmp_ssh.mkdir(parents=True, exist_ok=True)
    h_lab = lab / "ssh"
    for src_dir in (
        h_lab,
        repo / "os_build" / "cx2h2_linux_lab" / "ssh",
        repo / "os_build" / "cx2h_linux_lab" / "ssh",
        repo / "os_build" / "cx2g_linux_lab" / "ssh",
    ):
    ...
    if not result["ok"]:
        result["blocker"] = result.get("blocker") or "CX2H3_SSH_WAIT_TIMEOUT"
    return result
...
def stop_guest(repo: Optional[Path] = None, *, ssh_port: int = DEFAULT_SSH_PORT) -> Dict[str, Any]:
    repo = repo or repo_root_from_here()
    stopped = []
    pidfile = Path("/tmp/cx2h3-graphical/qemu.pid")
    if pidfile.is_file():
        try:
    ...
    try:
        ...
        p = Path("/tmp/gunnchos-cx-qemu.lock")
        if p.exists():
            p.unlink()
    except OSError:
        pass
    return {"ok": True, "stopped": stopped, "ssh_port": ssh_port}
