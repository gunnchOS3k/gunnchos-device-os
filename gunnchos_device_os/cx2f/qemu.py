...
from gunnchos_device_os.cx2f.lock import acquire_lock, release_lock
from gunnchos_device_os.cx2f.paths import (
    CX2E_OVERLAY_REL,
    cx2e_lab_root,
    cx2f_lab_root,
    ensure_lab_tree,
...
def ensure_ssh_keypair(lab: Path) -> Dict[str, str]:
    ...
    if not priv.is_file():
        # Prefer CX2E keys so provisioned overlay auth continues to work
        cx2e = cx2e_lab_root() / "ssh" / "id_ed25519"
        if cx2e.is_file():
            shutil.copy2(cx2e, priv)
            shutil.copy2(cx2e.with_suffix(".pub"), pub)
            os.chmod(priv, 0o600)
        else:
            subprocess.run(
                ["ssh-keygen", "-t", "ed25519", "-N", "", "-f", str(priv), "-C", "cx2f-lab"],
                check=True,
                capture_output=True,
                text=True,
            )
...
def prepare_overlay(repo: Path) -> Dict[str, Any]:
    """Clone CX2E overlay into CX2F namespace as qcow2 backing file (CX2E immutable)."""
    lab = ensure_lab_tree(repo)
    parent = repo / CX2E_OVERLAY_REL
    overlay = lab / "overlays" / "cx2f-aarch64.qcow2"
    prereq = host_prereqs()
    result: Dict[str, Any] = {
        "ok": False,
    ...
    if not parent.is_file():
        result["blocker"] = "CX2F_CX2E_PARENT_OVERLAY_MISSING"
        return result
    if not prereq["ok"]:
        result["blocker"] = f"CX2F_HOST_PREREQ_FAIL: {prereq}"
        return result
    ...
    if not overlay.is_file():
        ...
        try:
            ...
            result["blocker"] = f"CX2F_OVERLAY_CREATE_FAILED: {exc.stderr[-500:]}"
            return result
    result["overlay_sha256"] = sha256_file(overlay)
    result["ok"] = True
    result["cx2e_overlay_immutable"] = True
    return result
...
def build_graphical_cmd(
    *,
    ...
    return [
        ...
        "-drive",
        f"if=none,id=cx2f_disk,file={disk},format=qcow2",
        "-device",
        "virtio-blk-pci,drive=cx2f_disk,bootindex=1",
        "-netdev",
...
def start_graphical_guest(repo: Optional[Path] = None, *, ssh_port: int = DEFAULT_SSH_PORT) -> Dict[str, Any]:
    ...
    work = lab / "work"
    result: Dict[str, Any] = {
        "schema": "gunnchos.cx2f.graphical_boot.v1",
        "ok": False,
        "blocker": None,
        "ssh_port": ssh_port,
    }
    if any_qemu_running():
        # Bounded wait for foreign guest; never kill it.
        deadline = time.time() + 900
        while any_qemu_running() and time.time() < deadline:
            time.sleep(15)
        if any_qemu_running():
            result["blocker"] = "CX2F_QEMU_SLOT_BUSY_FOREIGN_PROCESS"
    ...
    lock = acquire_lock(
        branch="eng/cx2f-drm-shell-render-capture-journey-reearn",
        repo=str(repo),
        purpose="CX2F_DRM_SHELL_RENDER_CAPTURE",
    ...
    (work / "CX2F_QEMU_GRAPHICS.json").write_text(json.dumps(graphics, indent=2) + "\n")
    ...
    if proc.returncode != 0:
        ...
        result["blocker"] = f"CX2F_GRAPHICAL_BOOT_FAILED: {proc.stderr[-2000:]}"
        return result
    time.sleep(1)
    ...
    result["CX2F_QEMU_GRAPHICS_SURFACE_PASS"] = True  # VNC bound; validated further by screendump
    result["guest_booted"] = bool(pid and _pid_alive(pid))
    if not result["ok"]:
        result["blocker"] = result.get("blocker") or "CX2F_SSH_WAIT_TIMEOUT"
    return result
...
def stop_guest(repo: Optional[Path] = None, *, ssh_port: int = DEFAULT_SSH_PORT) -> Dict[str, Any]:
    ...
    stopped = []
    pidfile = Path("/tmp/cx2f-graphical/qemu.pid")
    if pidfile.is_file():
        try:
