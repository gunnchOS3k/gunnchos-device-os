...
FOREIGN_BUSY = "CX2F_QEMU_SLOT_BUSY_FOREIGN_PROCESS"
...
def acquire_lock(
    *,
    ...
    """Acquire CX QEMU slot. Only steal if prior lock PID is dead or owned by CX2F purpose."""
    ...
    while True:
        ...
        if cur:
            ...
            owned = str(cur.get("purpose", "")).startswith("CX2F") or str(cur.get("branch", "")).startswith(
                "eng/cx2f"
            )
            ...
            if alive and owned and pid != os.getpid():
                # prior CX2F holder still alive — wait then fail
                if time.time() >= deadline:
                    return {"ok": False, "blocker": FOREIGN_BUSY, "foreign": cur}
                time.sleep(poll_s)
                continue
...
def qemu_owned_by_cx2f(pidfile: Path, lock: Optional[Dict[str, Any]] = None) -> bool:
    lock = lock or read_lock()
    if not lock:
        return False
    if not str(lock.get("purpose", "")).startswith("CX2F"):
        return False
