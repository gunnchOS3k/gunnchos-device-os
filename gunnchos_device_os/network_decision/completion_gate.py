"""Strict Wave005 completion gate + negative-control injections."""
...
from gunnchos_device_os.network_decision.evaluators import (
    EVALUATORS,
    TARGET_REQUIREMENTS,
    run_all_evaluators,
)
...
def evaluate_completion_gate(
    ...
    *,
    ...
    """COMPLETE only when all 12 rows independently earn IMPLEMENTED_AND_VALIDATED."""
    ...
    reasons: list[str] = []
    if len(TARGET_REQUIREMENTS) != 12:
        reasons.append("target_count_not_12")
    if len(mapping) != 12:
        reasons.append("evaluator_map_count_not_12")
    if len(classification) != 12:
        reasons.append("result_count_not_12")
    if ids != EXPECTED:
    ...
    return {
        "schema": "gunnchos.engineering_wave005.completion_gate.v1",
...
def run_negative_controls() -> dict[str, Any]:
    """Injected broken/missing/empty evaluators must fail the completion gate."""
    base = run_all_evaluators()
    classification = base["classification"]
    results: dict[str, Any] = {}

    # 1) broken unconditional True with empty evidence
    ...
    # Optional env-based injection for CI/tests (does not mutate permanent registry)
    env_victim = os.environ.get("WAVE005_BROKEN_EVALUATOR")
    env_result = None
    if env_victim:
    ...
    return {
        "schema": "gunnchos.engineering_wave005.completion_gate_negative_control.v1",
        "ok": all_rejected,
