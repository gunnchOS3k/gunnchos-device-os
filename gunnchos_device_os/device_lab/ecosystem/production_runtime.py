...
def run_production_game(*, game_id: str, repo_root: Path, work: Path) -> dict[str, Any]:
    ...
    try:
        if not server.get("ok"):
            ...
            result["reason"] = "asset_server_failed"
            result["partial"] = True
            (work / f"{game_id}_production.json").write_text(json.dumps(result, indent=2) + "\n")
            (work / f"{game_id}_result.json").write_text(json.dumps(result, indent=2) + "\n")
            return result
        ...
        if not earned:
            result["partial"] = bool(runtime.get("partial"))
            result["reason"] = (
                runtime.get("reason")
                or runtime.get("error")
                or runtime.get("note")
                or ("viewport_without_input_save" if runtime.get("partial") else "chromium_runtime_incomplete")
            )
            if not chrome and not pw.get("ok"):
                result["reason"] = "no_chromium_or_playwright"
            elif chrome and not pw.get("ok") and result["runtime_class"] == "CHROMIUM_HEADLESS_VIEWPORT":
                result["chromium_reason"] = result["reason"]
    finally:
        result["cleanup"] = _stop_key(server.get("key") or "")

    if not result.get("FOUR_GAME_REAL_RUNTIME_EARNED") and not result.get("reason"):
        result["reason"] = (
            result.get("error")
            or (result.get("runtime_attempt") or {}).get("error")
            or (result.get("runtime_attempt") or {}).get("reason")
            or "production_runtime_not_earned"
        )
        result["partial"] = bool(result.get("partial") or result.get("runtime_attempt"))
