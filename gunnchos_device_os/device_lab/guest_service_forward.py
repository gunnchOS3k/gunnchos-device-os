...
class GuestForwardRule:
    ...
    def qemu_guestfwd_clause(self) -> str:
        ...
        return (
            f"guestfwd={proto}:{self.guest_addr}:{int(self.guest_port)}"
            f"-{proto}:{self.host_addr}:{int(self.host_port)}"
        )
...
def parse_guestfwd_env(raw: str | None = None) -> list[GuestForwardRule]:
    ...
    # Semicolon-separated full guestfwd= clauses or bare tcp:...-tcp:... forms.
    ...
    for i, chunk in enumerate(text.split(";")):
        ...
        if not chunk.startswith("tcp:") or "-tcp:" not in chunk:
