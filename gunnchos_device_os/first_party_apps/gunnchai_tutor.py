...
17G.6: prefers live gunnchAI product-service assist when sibling is up.
"""
...
CLAIM_BOUNDARY = (
    "Digital gunnchAI Tutor app with live product-service assist when available, "
    "local reply template fallback, prompt/response safety gates, sandbox session "
    "memory, and WAIKE lesson context binding. Not production LLM deployment, not "
    "frontier model quality, not HUMAN_E6."
)
...
def _require_live() -> bool:
    return os.environ.get("GUNNCHAI_REQUIRE_LIVE_ASSIST", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def run_gunnchai_tutor(
    *,
    ...
    session = {
        **session,
        "mock": False,
        "runtime": "product_service_or_template",
    ...
    reply_payload = tutor_assist_prefer_live(
        topic=topic,
        prompt=prompt,
        profile=profile,
    ...
    if reply_payload.get("provider_path") == "LIVE_PRODUCT_SERVICE":
        session["runtime"] = "product_service"
    elif "TEMPLATE" in str(reply_payload.get("provider_path") or ""):
        session["runtime"] = "local_template"
    ...
    turn = {
        ...
        "waike_lesson": bind_waike_lesson,
        "provider_path": reply_payload.get("provider_path"),
        "request_id": reply_payload.get("request_id"),
        "provenance": reply_payload.get("provenance"),
    }
    ...
    result = {
        ...
        "owner_functions": [
            ...
            "product_service_assist_or_local_template",
        ...
        "not_claimed": [
            ...
            "cx48_capability_broker_runtime",
        ],
    }
    runtime.write_json(data_dir / "gunnchai_tutor_run.json", result)
    return result
