"""CX2G journey re-earn — fail closed unless complete real UI/provider/read-back."""
...
def upgrade_journeys(facts: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    facts = facts or {}
    shell = bool(facts.get("CX2G_GUNNCH_SHELL_RENDER_PASS") or facts.get("shell_window_rendered"))
    capture = bool(facts.get("CX2G_QEMU_FRAMEBUFFER_CAPTURE_PASS") or facts.get("real_screen_capture_pass"))
    inp = bool(facts.get("CX2G_REAL_INPUT_TO_SHELL_MUTATION_PASS"))
    drm = bool(facts.get("CX2G_WESTON_DRM_PASS"))
    kernel = bool(facts.get("CX2G_NON_CLOUD_KERNEL_BOOT_PASS"))
    ...
    return {
        "schema": "gunnchos.cx2g.journey_upgrade.v1",
