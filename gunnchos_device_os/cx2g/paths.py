"""CX2G path helpers — isolated namespaces only."""
...
# Prefer CX2F overlay as immutable parent (proven DRM/Weston stack); fall back to CX2E.
CX2F_OVERLAY_REL = "os_build/cx2f_linux_lab/overlays/cx2f-aarch64.qcow2"
...
def evidence_root(repo: Optional[Path] = None) -> Path:
    return (repo or repo_root_from_here()) / "artifacts" / "complete_experience" / "cx2g"


def ensure_lab_tree(repo: Optional[Path] = None) -> Path:
    root = cx2g_lab_root(repo)
    for sub in (
        "config",
        "scripts",
        "work",
