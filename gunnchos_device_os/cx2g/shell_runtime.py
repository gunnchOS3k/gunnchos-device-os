"""ShellRuntimeTarget v1 for CX2G — loopback HTTP asset delivery."""
...
class ShellRuntimeTarget:
    schema: str = "gunnchos.cx2g.shell_runtime_target.v1"
    source_sha: str = ""
    build_hash: str = ""
    arch: str = "aarch64"
    ...
    display_socket: str = "/run/cx2g-wayland/wayland-0"
    dbus_session: str = "unix:path=/run/cx2g-wayland/bus"
    ozone_platform: str = "wayland"
    chromium_profile: str = "/var/lib/cx2g/chromium-shell"
...
def build_production_shell(repo: Path, out_dir: Path) -> Dict[str, Any]:
    ...
    if not (shell / "package.json").is_file():
        result["blocker"] = "CX2G_GUNNCH_SHELL_MISSING"
        return result
    ...
    if build.returncode != 0 or not (dist / "index.html").is_file():
        result["blocker"] = f"CX2G_SHELL_BUILD_FAILED: {(build.stderr or '')[-800:]}"
        return result
    ...
    target = ShellRuntimeTarget(
        ...
        launch_cmd=(
            ...
            "--user-data-dir=/var/lib/cx2g/chromium-shell "
