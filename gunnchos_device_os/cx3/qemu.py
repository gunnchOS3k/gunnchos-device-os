"""CX3 QEMU helpers — child overlay of CX2H.4; never kill foreign guests."""
...
def prepare_overlay(repo: Optional[Path] = None) -> Dict[str, Any]:
    ...
    overlay = lab / "overlays" / "cx3-aarch64.qcow2"
    parent = repo / CX2H4_OVERLAY_REL
    result: Dict[str, Any] = {"ok": False, "overlay": str(overlay), "parent": str(parent)}
    qimg = host_prereqs().get("qemu_img")
    if not qimg:
        result["blocker"] = "CX3_QEMU_IMG_MISSING"
        return result
    if not parent.is_file():
        result["blocker"] = "CX3_CX2H4_OVERLAY_MISSING"
        return result
    if overlay.is_file():
        result["ok"] = True
        result["reused"] = True
    ...
    if not result["ok"]:
        result["blocker"] = "CX3_OVERLAY_CREATE_FAILED"
    return result
...
def start_graphical_guest(repo: Optional[Path] = None, *, ssh_port: int = DEFAULT_SSH_PORT) -> Dict[str, Any]:
    ...
    lab = ensure_lab_tree(repo)
    result: Dict[str, Any] = {
        "schema": "gunnchos.cx3.graphical_boot.v1",
        "ok": False,
        "blocker": None,
        "ssh_port": ssh_port,
    }
    if cx_owned_qemu_running():
        ...
        while cx_owned_qemu_running() and time.time() < deadline:
            time.sleep(5)
        if cx_owned_qemu_running():
            result["blocker"] = "CX3_QEMU_SLOT_BUSY"
            return result
    ...
    lock = acquire_lock(
        branch="eng/cx3-credential-wallet-portfolio-foundation",
        repo=str(repo),
        purpose="CX3_CREDENTIAL_WALLET_PORTFOLIO",
    ...
    if proc.returncode != 0:
        ...
        result["blocker"] = f"CX3_GRAPHICAL_BOOT_FAILED: {proc.stderr[-2000:]}"
        return result
    time.sleep(1)
    ...
    if not result["ok"]:
        result["blocker"] = "CX3_SSH_TIMEOUT"
    return result
    ...
    if pid and _pid_alive(pid):
        ...
        if lock_path.is_file():
            try:
                ...
                if meta.get("qemu_pid") == pid or meta.get("wave") in ("CX3.1", "CX2H.4", "CX2H.3"):
                    owned = True
            except Exception:
                owned = True
        else:
