...
def iv_lab_001_path_escape() -> None:
    ...
    record(
        Finding(
            ...
            remediation_holds=ok,
            notes="Allowlist policy must not accept artifacts/ESCAPE_TEST",
        )
    )
...
def iv_lab_002_unregistered_tmp_denied() -> None:
    ...
    denied = False
    reason = ""
    with tempfile.TemporaryDirectory(prefix="vp007-iv-unreg-") as td:
        work = Path(td) / "inst"
        try:
    ...
    record(
        Finding(
            ...
            evidence={"denied": denied, "reason": reason},
            remediation_holds=ok,
            notes="WP007-IV-DEF-001: allowlist requires explicit register",
        )
    )
...
def iv_lab_003_host_root_not_approvable() -> None:
    ...
    record(
        Finding(
            ...
            evidence={"denied": denied, "reason": reason},
            remediation_holds=ok,
            notes="Allowlist must not approve host-sensitive roots",
        )
    )
...
def iv_lab_004_registered_tmp_allowed_escape_still_denied() -> None:
    ...
    registered_ok = False
    started_id = None
    with tempfile.TemporaryDirectory(prefix="vp007-iv-reg-") as td:
        tmp = Path(td)
    ...
    record(
        Finding(
            ...
            remediation_holds=ok,
            notes="CI allowlist adaptation must not weaken SEC-LAB escape denial",
        )
    )
