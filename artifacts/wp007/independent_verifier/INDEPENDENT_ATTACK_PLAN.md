...
| IV-LAB-002 | Device Lab | unregistered pytest tmp_path still denied under allowlist policy |
| IV-LAB-003 | Device Lab | host-sensitive root (`/etc`) cannot be registered |
| IV-LAB-004 | Device Lab | registered controlled temp allowed; escape sibling still denied |
...
Allowlist must not re-enable host escape (WP007-IV-DEF-001 remediation check).
